...
class Jianying_controller:
    ...
    def export_draft(self, draft_name: str, output_path: Optional[str] = None, *,
        ...
        **注意: 需要确认有导出草稿的权限(不使用VIP功能或已开通VIP), 否则可能陷入死循环**
        ...
        if kou_tu:
            ...
            self.click_button_by_move(os.path.join(self.current_dir, "combinations/rpa/template/ZhiNengKouXiang.png"))
            # 移动到智能抠像选择框
            time.sleep(1)
    ...
    def click_button(png_path):
        # confidence 识别灵敏度
        button_location = pyautogui.locateOnScreen(png_path, confidence=0.8)
        # 检查是否找到图像
        if button_location is not None:
            # 使用Box对象的属性获取中心点
            button_center = (button_location.left + button_location.width // 2,
